...
module.exports = function({ minified, es6, tests, coverage, demo, server }) {
  ...
  return {
    input: demo ? 'demo/index.ts' : 'src/iframe.ts',
    external: tests || server ? [] : ['@salte-auth/salte-auth'],
    output: {
      file: `dist/iframe${minified ? '.min' : ''}.${es6 ? 'mjs' : 'js'}`,
      format: es6 ? 'es' : 'umd',
      name: 'salte.auth.iframe',
      ...
      banner: deindent`
         ...
         *
         * @license MIT (https://github.com/salte-auth/iframe/blob/master/LICENSE)
         *
